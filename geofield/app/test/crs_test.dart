...
void main() {
  ...
  group('зоны Гаусса-Крюгера', () {
    ...
    test('покрытие Магадан + Якутия: долготы ложатся в зоны 18–28', () {
      // Крайний запад Якутии ~102° → зона 18; восток Магадана ~168° → 28.
      expect(gkZone(105.0), 18);
      expect(gkZone(114.0), 20); // Мирный
      expect(gkZone(129.73), 22); // Якутск
      expect(gkZone(148.16), 25); // Сусуман
      expect(gkZone(149.63), 25); // Тенькинский (Усть-Омчуг) — ещё зона 25
      expect(gkZone(150.5), 26); // восточнее 150° — зона 26
      expect(gkZone(160.0), 27);
      expect(gkCentralMeridian(22), 129.0);
      expect(gkCentralMeridian(27), 159.0);
    });
  });
  ...
  group('обратимость (round-trip)', () {
    ...
    // Реальная зона работ партии — вся Магаданская обл. и Якутия (зоны 18–28).
    test(
        'Тенькинский р-н (Усть-Омчуг ~149.5°)', () => roundTrip(61.15, 149.63));
    test('Якутск (~129.7°, зона 22)', () => roundTrip(62.03, 129.73));
    test('запад Якутии (Мирный ~114°, зона 20)', () => roundTrip(62.53, 114.0));
    test('крайний запад Якутии (~105°, зона 18)', () => roundTrip(60.0, 105.5));
    test('восток Магаданской обл. (~160°, зона 27)',
        () => roundTrip(62.5, 160.0));
    test('сплошной прогон долгот 105–162° (Якутия→Магадан)', () {
      for (var lon = 105.0; lon <= 162.0; lon += 3.0) {
        roundTrip(62.0, lon);
      }
    });
