...
2026-08-22_reset_code_issue.sql                  # атомарная выдача reset-кода на пользователя
2026-08-22_bind_code_issue.sql                   # атомарная выдача bind-кода на пользователя
2026-08-23_reset_code_email_guard.sql             # повторная сверка актуального recovery email
2026-08-23_reset_code_email_lock.sql              # row lock для reset email TOCTOU
```

Августовские файлы не меняют июльско-августовскую основную цепочку, но recovery-кодовые
миграции нужно применять в порядке, указанном ниже: каждая следующая пересоздаёт ту же RPC
и добавляет проверку к предыдущей редакции.
...
`2026-08-22_reset_code_issue.sql` ставится после `2026-07-17_recovery_email_auth_codes.sql`.
Она добавляет закрытую service-role функцию для атомарной выдачи reset-кода: проверка окна,
гашение старых кодов и вставка нового выполняются под advisory lock пользователя, поэтому
параллельные запросы не отправляют несколько взаимоисключающих кодов.

`2026-08-22_bind_code_issue.sql` ставится после той же миграции и применяет тот же принцип
к подтверждению резервной почты: параллельный запрос получает `wait`, а письмо уходит только
после успешной вставки единственного актуального кода.

`2026-08-23_reset_code_email_guard.sql` ставится после `2026-08-22_reset_code_issue.sql`.
Она обновляет ту же service-role RPC: перед выдачей кода текущий подтверждённый адрес
повторно читается из `user_recovery`, чтобы устаревший запрос не отправил письмо на прежнюю почту.

`2026-08-23_reset_code_email_lock.sql` ставится после `2026-08-23_reset_code_email_guard.sql`.
Она удерживает строку `user_recovery` через `FOR UPDATE` до вставки reset-кода, синхронизируя
выдачу кода с подтверждением и отвязкой адреса.
