-- 2026-08-12 — БАЗОВАЯ RLS-обвязка четырёх таблиц, которой не было в репозитории.
--
...
--   • не трогает журнал, склад, историю, org-роли и партии — их политики в каталоге есть.
...
declare
  ...
  turned text[] := '{}';
begin
  foreach t in array array['bases', 'base_members', 'profiles', 'tasks'] loop
  ...
  if array_length(turned, 1) is null then
    raise notice 'RLS: уже включён на всех четырёх таблицах.';
  else
